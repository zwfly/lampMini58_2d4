                  ...
                  uint32_t u32BusClock)
{
    ...
    spi->CTL = u32MasterSlave | (u32DataWidth << (3)) | (u32SPIMode);

    if(u32MasterSlave == ((0x1ul << (18))))
        spi->SSCTL = (0x1ul << (4));
...
void SPI_Close(SPI_T *spi)
{
    ...
    ((SYS_T *) (((uint32_t)0x50000000) + 0x00000))->IPRST1 |= (0x1ul << (12));
    ((SYS_T *) (((uint32_t)0x50000000) + 0x00000))->IPRST1 &= ~(0x1ul << (12));
}
...
void SPI_EnableAutoSS(SPI_T *spi, uint32_t u32SSPinMask, uint32_t u32ActiveLevel)
{
    spi->SSCTL = (spi->SSCTL & ~((0x1ul << (2)) | (0x1ul << (0)))) | (u32SSPinMask | u32ActiveLevel) | (0x1ul << (3));
}
...
uint32_t SPI_SetBusClock(SPI_T *spi, uint32_t u32BusClock)
{
    ...
    if((((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKSEL1 & (0x3ul << (4))) == 0x00000000UL) {
        if((u32ClkSrc = CLK_GetHXTFreq()) == 0)
            u32ClkSrc = CLK_GetLXTFreq();
    } else if((((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKSEL1 & (0x3ul << (4))) == 0x00000020UL)
    ...
    spi->CLKDIV = (spi->CLKDIV & ~(0xfful << (0))) | u32Div;
...
void SPI_EnableFIFO(SPI_T *spi, uint32_t u32TxThreshold, uint32_t u32RxThreshold)
{
    spi->FIFOCTL = (spi->FIFOCTL & ~((0x3ul << (28)) | (0x3ul << (24))) |
                    (u32TxThreshold << (28)) |
                    (u32RxThreshold << (24)));
...
void SPI_EnableInt(SPI_T *spi, uint32_t u32Mask)
{
    ...
    if((u32Mask & (0x40)) == (0x40))
        spi->FIFOCTL |= (0x1ul << (21));
}
